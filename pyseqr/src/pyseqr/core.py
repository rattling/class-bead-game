...
class OccurrenceGapStrategy(ABC):
    ...
    @abstractmethod
    def filter_indices(
        self, indices: List[int], last_occurrence_max: int
    ) -> List[int]:
        """
...
def validate_inputs(sublist: Sequence[Any], target: Sequence[Any]) -> None:
    ...
    """
    if not isinstance(sublist, (list, tuple)) or not isinstance(
        target, (list, tuple)
    ):
        raise TypeError(
            "Both sublist and target must be of type list or tuple."
        )
...
def round_as_decimal(num, decimal_places=2):
    ...
    return Decimal(str(num)).quantize(
...
def make_hashable(
    ...
    if use_custom_str:
        ...
        if type(element).__module__ != "builtins":
            ...
            if hasattr(element, "__str__") and callable(
                getattr(element, "__str__")
            ):
                return str(element)
            else:
                raise TypeError(
...
def find_in_list(
    ...
    def get_occurrence_gap_strategy(
        flag: OccurrenceGap | str,
    ...
    while True:
        ...
        for element in sublist:
            # 1) Filter indices e.g. may disallow overlaps depending on the strategy
